...
INTO #dev2Alldata
from  #ZeroToSixM2 t0
left join #SixToTweM2 t6 on t0.salername2=t6.salername2
LEFT JOIN #AboveTwe2 t12 on t12.salername2=t0.salername2
where  t0.salername2 LIKE '%'+@salername+'%' OR  t0.salername2 IN (SELECT salername FROM #tbSalerName)
...
select
'归属1人表' as tableType,
max(timegroupZero) as timegroupZero,
'A汇总' as salernameZero ,
sum(salemoneyrmbusZero) as salemoneyrmbusZero,
sum(salemoneyrmbznZero) as salemoneyrmbznZero,
sum(costmoneyrmbZero) as costmoneyrmbZero,
sum(ppebayusZero) as ppebayusZero,
sum(ppebayznZero) as ppebayznZero,
sum(inpackagefeermbZero) as inpackagefeermbZero,
sum(expressfarermbZero) as expressfarermbZero,
sum(devofflinefeeZero) as devofflinefeeZero,
sum(devOpeFeeZero) as devOpeFeeZero,
sum(netprofitZero) as netprofitZero,
--sum(netrateZero) as netrateZero,
CASE WHEN sum(salemoneyrmbznZero)=0 THEN 0
ELSE
cast(sum(netprofitZero)/sum(salemoneyrmbznZero)*100  as DECIMAL(20,2))
END as netrateZero,

max(timegroupSix) as timegroupSix,
sum(salemoneyrmbusSix) as salemoneyrmbusSix,
sum(salemoneyrmbznSix) as salemoneyrmbznSix,
sum(costmoneyrmbSix) as costmoneyrmbSix,
sum(ppebayusSix) as ppebayusSix,
sum(ppebayznSix) as ppebayznSix,
sum(inpackagefeermbSix) as inpackagefeermbSix,
sum(expressfarermbSix) as expressfarermbSix,
sum(devofflinefeeSix) as devofflinefeeSix,
sum(devOpeFeeSix) as devOpeFeeSix,
sum(netprofitSix) as netprofitSix,
--sum(netrateSix) as netrateSix,
CASE WHEN sum(salemoneyrmbznSix)=0 THEN 0
ELSE
cast(sum(netprofitSix)/sum(salemoneyrmbznSix)*100  as DECIMAL(20,2))
END as netrateSix,

max(timegroupTwe) as timegroupTwe,
sum(salemoneyrmbusTwe) as salemoneyrmbusTwe,
sum(salemoneyrmbznTwe) as salemoneyrmbznTwe,
sum(costmoneyrmbTwe) as costmoneyrmbTwe,
sum(ppebayusTwe) as ppebayusTwe,
sum( ppebayznTwe) as ppebayznTwe,
sum(inpackagefeermbTwe) as inpackagefeermbTwe,
sum(expressfarermbTwe) as expressfarermbTwe,
sum(devofflinefeeTwe) as devofflinefeeTwe,
sum(devOpeFeeTwe) as devOpeFeeTwe,
sum(netprofitTwe) as netprofitTwe,
--sum(netrateTwe) as netrateTwe,
CASE WHEN sum(salemoneyrmbznTwe)=0 THEN 0
ELSE
cast(sum(netprofitTwe)/sum(salemoneyrmbznTwe)*100  as DECIMAL(20,2))
END as netrateTwe,
sum(salemoneyrmbtotal) as salemoneyrmbtotal,
sum(netprofittotal)as netprofittotal,
CASE WHEN sum(salemoneyrmbtotal)=0 THEN 0
ELSE
cast(sum(netprofittotal)/sum(salemoneyrmbtotal)*100  as DECIMAL(20,2))
END as netratetotal
...
CASE WHEN sum(salemoneyrmbznZero)=0 THEN 0
ELSE
cast(sum(netrateSix)/sum(salemoneyrmbznSix)*100  as DECIMAL(20,2))
END as netrateSix,
max(timegroupTwe) as timegroupTwe,
sum(salemoneyrmbusTwe) as salemoneyrmbusTwe,
sum(salemoneyrmbznTwe) as salemoneyrmbznTwe,
...
CASE WHEN sum(salemoneyrmbznTwe)=0 THEN 0
ELSE
cast(sum(netrateTwe)/sum(salemoneyrmbznTwe)*100  as DECIMAL(20,2))
END as netrateTwe,
sum(salemoneyrmbtotal) as salemoneyrmbtotal,
sum(netprofittotal)as netprofittotal,
CASE WHEN sum(salemoneyrmbtotal)=0 THEN 0
